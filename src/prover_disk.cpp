...
void DiskProver::GetMemo(uint8_t* buffer)
{
    memcpy(buffer, memo, this->memo_size);
}

uint32_t DiskProver::GetMemoSize() const noexcept
{
    return this->memo_size;
}

void DiskProver::GetId(uint8_t* buffer)
{
    memcpy(buffer, id, kIdLen);
}

std::string DiskProver::GetFilename() const noexcept
{
    return filename;
}

uint8_t DiskProver::GetSize() const noexcept
{
    return k;
}
...
std::vector<Bits> DiskProver::GetInputs(std::ifstream& disk_file, uint64_t position, uint8_t depth)
{
